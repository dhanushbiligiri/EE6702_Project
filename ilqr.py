...
class ILQR:
    ...
    def backward_pass(
        ...
        for t in reversed(range(T)):
            ...
            # maximize reward -> want Quu negative definite
            Quu_reg = Quu - reg * np.eye(act_dim)
    ...
    def optimize(
        ...
        cov_best = np.stack([np.eye(act_dim, dtype=np.float64) * 1e-2 for _ in range(T)], axis=0)
        cov_inv_best = np.stack([np.eye(act_dim, dtype=np.float64) * 1e2 for _ in range(T)], axis=0)
        ...
        for i in iterator:
            if show_progress:
                tqdm.write(f"[iLQR] iter={i} | current_best_return={best_return:.4f} | reg={reg:.2e}")
            ...
            while reg <= self.cfg.reg_max:
                try:
                    ...
                    if show_progress:
                        tqdm.write(f"  backward pass failed, increasing reg -> {reg:.2e}")
                    continue
                ...
                if improved and best_trial is not None:
                    ...
                    if show_progress:
                        tqdm.write(f"  accepted step | new_best_return={best_return:.4f} | reg={reg:.2e}")
                    success = True
                    break
                else:
                    reg *= self.cfg.reg_scale_up
